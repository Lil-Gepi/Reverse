...
sign_sites <- res_r11_s1[which(res_r11_s1$P<0.05 & res_r11_s1$P > 0.01),]; row.names(sign_sites) <- sign_sites$SNP
top_sites <- res_r11_s1[order(res_r11_s1$P),][1:100,]; row.names(top_sites) <- top_sites$SNP
low_sites <- res_r11_s1[which(res_r11_s1$P>0.05),]; low_sites <- low_sites[sort(sample.int(nrow(low_sites), 100)), ]; row.names(low_sites) <- low_sites$SNP
...
sign_sites_af <- Allele_f[rownames(sign_sites),1:2]
top_sites_af <- Allele_f[rownames(top_sites),1:2]
low_sites_af <- Allele_f[rownames(low_sites),1:2]
...
### plotting
sign_sites_af_start <- cbind(SNP = rownames(sign_sites_af), Freq = sign_sites_af[,1], State = 0)
sign_sites_af_end <- cbind(SNP = rownames(sign_sites_af), Freq = sign_sites_af[,2], State = 30)
...
tiff("sign_SNPs_AFC_s11.tiff",  width = 18, height = 10, res = 300, units = "cm")
print(sign_plot)
dev.off()

top_sites_af_start <- cbind(SNP = rownames(top_sites_af), Freq = top_sites_af[,1], State = 0)
top_sites_af_end <- cbind(SNP = rownames(top_sites_af), Freq = top_sites_af[,2], State = 30)
...
tiff("top_SNPs_AFC_s11.tiff",  width = 18, height = 10, res = 300, units = "cm")
print(top_plot)
dev.off()


low_sites_af_start <- cbind(SNP = rownames(low_sites_af), Freq = low_sites_af[,1], State = 0)
low_sites_af_end <- cbind(SNP = rownames(low_sites_af), Freq = low_sites_af[,2], State = 30)
...
tiff("low_SNPs_AFC_s11.tiff",  width = 18, height = 10, res = 300, units = "cm")
print(low_plot)
dev.off()